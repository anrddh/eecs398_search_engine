...
namespace FB 
   ...
   struct Node 
      {
         template< class... Args >
         Node( Args&&... args ) : val(std::forward<Args>(args)...), left(nullptr), right(nullptr), parent(nullptr) { }
         Node( T t ) : val( t ), left( nullptr ), right( nullptr ), parent( nullptr ) { }
   ...
   class Set 
      ...
      public:
         class Iterator
            ...
            public:
               friend class Set;
               Iterator( Node<T> * root_, Node<T> * current_ ) : root( root_ ), current( current_ ) { }
         ...
         Set( Set &other ) : root(nullptr), sz(0)
            {
            for ( T val : other )
               {
               insert( val );
         ...
         std::pair< Iterator, bool > emplace( Args&&... args ) 
            {
               Node< T > * emplNode = new Node<T>(std::forward<Args>(args)...);
               if ( !root )
                  {
                  ++sz;
                  root = emplNode;
         ...
         Iterator erase( T val ) 
            ...
            if( !comp( val, root->val ) && !comp( root->val, val ) )
               ...
               root = eraseNode( root );
               root->parent = nullptr;
               return Iterator( root, findNext( root ) );
               }
         ...
         if(erase == root)
            ...
            if ( parent->left == erase )
               ...
               return Iterator( root, findNext( parent ) );
               }
            }
         }
         ...
         Iterator begin( ) 
            {
            Node< T > * n = root;
            return Iterator(root, findFirst(root));
            }
      ...
      private:
         ...
         Iterator insertImpl( Node< T > * node, T val )
            {
         ...
         Node< T > * insertNode( Node< T > * node, T val, bool left )
            {
            ++sz;
         ...
         std::pair<Iterator, bool> emplaceImpl( Node< T > * node,  Node< T > * emplNode ) 
            {
            ...
            Node< T > * emplaceNode( Node< T > * node, Node< T > * emplNode, bool left )
            {
            ++sz;
         ...
         Iterator eraseImpl( Node< T > * node, T val, bool left )
            {
            if( left )
               {
               if( !node->left )
               ...
               if( !node->right )
                  {
                  // no value to erase, find correct Iterator to return
                  return Iterator(root, findNext( node ) );
                  }
         ...
         Node<T> * eraseNode( Node< T > * node)
            ...
            if ( !node->left || !node->right )
               ...
               {
               output = findPrevious( node );
               Node<T> * parent = output->parent;
               if( parent == node )
                  {
         ...
         Iterator findImpl( Node< T > * node, T val)
            {
            if( !comp( val, node->val ) && !comp( node->val, val ) )
               {
               return Iterator( root, node );
